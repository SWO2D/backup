...
contract NFTMarketplace is AccessControlEnumerable, ReentrancyGuard, IERC721Receiver
{
    ...
    /** Contract
        1. ShellCoin: ERC20 contract used as the payment currency in the NFTmarketplace
        2. ShellBoxesNFTcollection: ERC721 contract whose NFTs will be traded in this NFTmarketplace.
    */
    IERC20 public immutable ShellCoin;
    IERC721 public immutable ShellBoxesNFTcollection;
    ...
    // Sell order: ShellBoxesNFTcollectionID -> Order
    ...
    constructor(
        ...
        // Contracts
        ShellCoin = IERC20(erc20token);
        ShellBoxesNFTcollection = IERC721(nftToken);
        // Roles
    ...
    /**
     *
     * @dev Lists a buy order for a given user by taking the ShellCoins as collateral to fulfill the order
     *
    */
        ...
        nonReentrant
    {
        ...
        require(
            ShellBoxesNFTcollection.ownerOf(nftId) != address(0),
            "nftID does not exists"
        );
        // require that the erc20AmountOffer is greater than 0
        require(erc20AmountOffer > 0, "Offer should be > 0");
        // transfer ShellCoins to the contract
        require(
            ShellCoin.transferFrom(_msgSender(), address(this), erc20AmountOffer),
            "ShellCoin transfer failed"
        );
        // order creation
    ...
    */
    function cancelBuyOrder(uint256 orderId) external nonReentrant {
        ...
        //transfer back the ShellCoin initially put as collateral
        require(
            ShellCoin.transfer(_msgSender(), order.amount),
            "ShellCoin transfer failed"
        );
        order.status = OrderStatus.Cancelled;
    ...
    */
        ...
        nonReentrant
    {
        ...
        require(
            ShellCoin.transferFrom(_msgSender(), address(this), increaseAmount),
            "ShellCoin transfer failed"
        );
        // increase the order
        ...
        nonReentrant
    {
        ...
        require(
            ShellCoin.transfer(_msgSender(), decreaseAmount),
            "ShellCoin transfer failed"
        );
        order.amount -= decreaseAmount;
    ...
    /**
     *
     * @dev Creates a sell order for the ShellBoxesNFT
     *
     * @param nftId NFT Id to sell
     * @param amount amount of ShellCoins to list for
     *
    ...
    */
        ...
        nonReentrant
    {
        ...
        ShellBoxesNFTcollection.safeTransferFrom(
            ShellBoxesNFTcollection.ownerOf(nftId),
            address(this),
            nftId,
            bytes("COLLATERAL")
        );
        ...
        require(
            ShellBoxesNFTcollection.ownerOf(nftId) == address(this),
            "ShellBoxesNFTcollection: ownership"
        );
    ...
    function cancelSellOrder(uint256 nftId) external nonReentrant {
        ...
        require(
            ShellBoxesNFTcollection.ownerOf(nftId) == _msgSender(),
            "ShellBoxesNFTcollection: ownership 2"
        );
        order.status = OrderStatus.Cancelled;
    ...
    /**
     *
     * @dev purchases a ShellBoxesNFT for the listed sell order price
     *
     * @param nftId the ShellBoxesNFT Id to purchase
     *
     * @notice requires a valid ShellBoxesNFT with an active sell order;
    ...
    function buySellOrder(uint256 nftId) external nonReentrant {
        ...
        // transfer ShellCoins to order owner
        require(
            ShellCoin.transferFrom(_msgSender(), order.owner, order.amount),
            "ShellCoin transfer failed"
        );
        // transfer the ShellBoxesNFT to the buyer
        ShellBoxesNFTcollection.safeTransferFrom(
            address(this),
            _msgSender(),
            nftId,
            bytes("PURCHASE ShellBoxesNFT")
        );
    ...
    /**
     *
     * @dev used to 'sell' to a listed buy order by an owner of a ShellBoxesNFT
     *
    ...
    function sellToOrderId(uint256 orderId) external nonReentrant {
        ...
        //require the caller to own the ShellBoxesNFT for this orderId
        require(
            ShellBoxesNFTcollection.ownerOf(order.nftId) == _msgSender(),
            "ShellBoxesNFTcollection: ownership"
        );
        // transfer to seller
        require(
        ...
        //require new ownership
        require(
            ShellBoxesNFTcollection.ownerOf(order.nftId) == order.owner,
            "ShellBoxesNFTcollection: ownership"
        );
        order.status = OrderStatus.Fulfilled;
    ...
    /**
     *
     * @dev Used to 'bid' for a ShellBoxesNFT with Ether
     *
     * @param nftId nftId of the ShellBoxesNFTcollection
     *
     */
    ...
    /**
     *
     * @dev Used to 'accept a Ether bid' for a ShellBoxesNFT
     *
     * @param nftId nftId of the ShellBoxesNFTcollection
     *
     */
